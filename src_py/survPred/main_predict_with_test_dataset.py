...
src_root = os.path.join(root,"src_py")
...
parser.add_argument('-batch','--batch_tag', required=False, type=str, default='TCGA')
...
parser.add_argument('-split','--split', required=False, type=str, default='test') # choices= fold0-4, foldBFsplit, Resample01-20
...
# parser.add_argument('-addTumorSegTask','--addTumorSegTask', required=False, type=bool, default=False)
...
expe_config = config.set_experiment_config(
    ...
    # addTumorSegTask = args.addTumorSegTask,
    debug = False
)
...
if model_config.model_loc is None:
    ...
    for split_tag in ['test']:
        ...
        pred_out = predict.predict(expe_config, cases, model, model_config, pred_out_dir, mode='test_offline',batch_tag=batch_tag) # test_online
