...
class ASPPModule(nn.Module):
    ...
    def __init__(self, features, inner_features=32, out_features=64, dilations=(12, 24, 36)):
        ...
        self.conv1 = nn.Sequential(nn.AdaptiveAvgPool3d((1,1,1)),
                                   ...
                                   norm(inner_features),
                                   nn.ReLU(inplace=True))
        self.conv2 = nn.Sequential(nn.Conv3d(features, inner_features, kernel_size=1, padding=0, dilation=1, bias=False),
                                   norm(inner_features),
                                   nn.ReLU(inplace=True))
        self.conv3 = nn.Sequential(nn.Conv3d(features, inner_features, kernel_size=3, padding=dilations[0], dilation=dilations[0], bias=False),
                                   norm(inner_features),
                                   nn.ReLU(inplace=True))
        self.conv4 = nn.Sequential(nn.Conv3d(features, inner_features, kernel_size=3, padding=dilations[1], dilation=dilations[1], bias=False),
                                   norm(inner_features),
                                   nn.ReLU(inplace=True))
        self.conv5 = nn.Sequential(nn.Conv3d(features, inner_features, kernel_size=3, padding=dilations[2], dilation=dilations[2], bias=False),
                                   norm(inner_features),
                                   nn.ReLU(inplace=True))
...
class CSAMbasicBlock(nn.Module):
    ...
    def __init__(self, in_chan, out_chans_list, stride,reduction=None, if_CA=False,if_SA=False,if_CAAM=False, bin_size = 4):
        # # Arguments
...
class LiverSurv(nn.Module):
    ...
    def __init__(self, task_names,n_phase=2, clin=False):
        ...
        self.Conv_in_ph1 = nn.Sequential(
            ...
            Conv3dSame(2,outChan_list[0],kernel_size=3,stride=stride_list[0], bias=False),
            norm(outChan_list[0]),
            activation(),
        ...
        if self.n_phase==1:
            ...
            self.convBlock_fuse = nn.Sequential(
                nn.Conv3d(outChan_list[4]*2, 256, kernel_size=1, bias=False),
                nn.BatchNorm3d(256),
                nn.ReLU(inplace=True),
        ...
        self.decoder_deepest = nn.Sequential(
            nn.Linear(126720,128), # 13440, 73728, 36864, 7680
            activation(),
            nn.Linear(128,64),
            activation()
        )
    ...
    def forward(self, x_ph1, x_ph2=None,clin_data = None):
        ...
        model_res["cam"]={"A":[],"V":[]} # [] # uncomment only for computing CAM, otherwise causes CUDA OOM.
        ...
        ### uncomment only for computing CAM, otherwise causes CUDA OOM.
        model_res["cam"]["A"].append(x1_ph1)
        model_res["cam"]["V"].append(x1_ph2)
        ...
        x1 = self.decoder_deepest(x1)
        x1 = torch.cat(scale_fc_list+[x1], dim=1)
        ...
        for i in self.task_names:
            ...
            decoder_out = torch.flatten(decoder_out, 1)
