...
def crop_tumorBbox_3d(caseName):
...
if __name__ == '__main__':
    ...
    save_root = os.path.join(proj_root, 'data_cleaned_CECT_annotations/tumorROI_3d_liverNoDilate_xyzMedian_no_zscore') # based on max tumorMaskGT
    ...
    for bt in batch_tags:
        ...
        caselist.sort()
        liver_mask_path = os.path.join(proj_root,'data_cleaned_CECT_annotations/livermask/{}_byModel_postprocessed/'.format(bt))  # final_liver byModel_postprocessed
        # tumor_mask_path = os.path.join(proj_root,'data_cleaned_CECT_annotations/cleaned_{}_allTumorAnnotations/'.format(bt))
        tumor_mask_path = os.path.join(proj_root,'data_cleaned_CECT_annotations/cleaned_{}_maxTumorAnnotation/'.format(bt))
        ...
        # caseName = caselist[0]
        results = pool.map(crop_tumorBbox_3d, caselist)
    ...
    for bt in batch_tags:
        ...
        # ----- compute liverboxes for batch b1to2 - ------
        ...
        #
        #
        # ----- compute liverboxes for batch b3 - ------
        # for b3, the max shape=[392.0, 346.0, 46.0]
        # for b3, the median shape=[293.0, 247.0, 31.0]
        # for b3, the min shape=[219.0, 156.0, 14.0]
        #
        #
        # ----- compute liverboxes for batch b4 - ------
        # for b4, the max shape=[450.0, 335.0, 45.0]
        # for b4, the median shape=[294.0, 245.0, 30.0]
        # for b4, the min shape=[194.0, 153.0, 15.0]
        #
        # ----- compute liverboxes for batch HUZHOU - ------
        # for HUZHOU, the max shape=[408.0, 302.0, 40.0]
        # for HUZHOU, the median shape=[279.0, 241.0, 30.0]
        # for HUZHOU, the min shape=[220.0, 191.0, 20.0]
        #
        #
        # ----- compute liverboxes for batch TCGA - ------
        # for TCGA, the max shape=[467.0, 317.0, 46.0]
        # for TCGA, the median shape=[336.0, 254.5, 34.5]
        # for TCGA, the min shape=[188.0, 198.0, 28.0]
        #
        #
        # ----- compute liverboxes for batch SRRH - ------
        # for SRRH, the max shape=[403.0, 327.0, 43.0]
        # for SRRH, the median shape=[292.5, 249.0, 30.0]
        # for SRRH, the min shape=[195.0, 184.0, 17.0]
        #
        #
        # ----- compute tumorboxes for batch b1to2 - ------
        # for b1to2, 0 pv mask contain no tumor, 0 art mask contain no tumor
        # for b1to2, the max shape=[316, 251, 32]
        # for b1to2, the median shape=[57.0, 59.0, 6.0]
        # for b1to2, the min shape=[16, 16, 1]
        #
        #
        # ----- compute tumorboxes for batch b3 - ------
        # for b3, 0 pv mask contain no tumor, 0 art mask contain no tumor
        # for b3, the max shape=[247, 195, 27]
        # for b3, the median shape=[54.0, 55.0, 5.0]
        # for b3, the min shape=[9, 9, 0]
        #
        #
        # ----- compute tumorboxes for batch b4 - ------
        # for b4, 0 pv mask contain no tumor, 0 art mask contain no tumor
        # for b4, the max shape=[173, 263, 33]
        # for b4, the median shape=[49.0, 50.0, 5.0]
        # for b4, the min shape=[14, 13, 0]
        #
        # ----- compute tumorboxes for batch HUZHOU - ------
        # for HUZHOU, 0 pv mask contain no tumor, 0 art mask contain no tumor
        # for HUZHOU, the max shape=[169, 220, 27]
        # for HUZHOU, the median shape=[53.5, 48.5, 5.0]
        # for HUZHOU, the min shape=[24, 23, 1]
        #
        #
        # ----- compute tumorboxes for batch TCGA - ------
        # for TCGA, 0 pv mask contain no tumor, 0 art mask contain no tumor
        # for TCGA, the max shape=[204, 231, 40]
        # for TCGA, the median shape=[105.5, 115.5, 12.0]
        # for TCGA, the min shape=[27, 25, 1]
        #
        #
        # ----- compute tumorboxes for batch SRRH - ------
        # for SRRH, 0 pv mask contain no tumor, 0 art mask contain no tumor
        # for SRRH, the max shape=[312, 265, 42]
        # for SRRH, the median shape=[62.0, 60.0, 7.0]
        # for SRRH, the min shape=[14, 14, 0]
